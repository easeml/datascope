            ...
            for D_c in Dp:
                ...
                if(D_c[2] != q and D_c[0] >= D_b[0]):
                    ...
                    for h in range(n_unit):
                        if h != i-1 and h != j-1 and h != q-1 and t.too_far_set(d[h], D_b, D_c):
                            tallies = t.copy_vector(t.append_tally(tallies, d[h], D_b, D_c, L_t))
                    
                    # We add the tally vector value of unit j     
                    tallies = t.copy_vector(t.append_tally(tallies, d[j-1], D_b, D_c, L_t))
                    ...
                    if((i == q and i != j) or (i != q and i == j)):
                        ...
                        m = 1
                        tallies = t.copy_vector(t.append_tally(tallies, d[i-1], D_b, D_c, L_t))
                        ...
                        for l2 in range(k+1):
                            ...
                            for a in range(m,k+1):
                                for b in range(n_out_set+1):
                                    if(a-b >= m):
                                        DP_val = DP_val + t.choose(n_out_set, b)*DP[l][a-b][l1][l2][k-l1][k-l2]/t.choose(n_unit-1, a)
